...
local V = {
				...
				Author = "ItzDannio25",
				Information = "tits",
				Model =	"models/dannio/ds7.mdl",

				KeyValues = {				
								vehiclescript =	"scripts/vehicles/dannio/ds7.txt"
					    }
}

list.Set( "Vehicles", "dannio_2018_ds7_crossback", V )

if !S_L_Josuneds7_NEWAddProps_List then S_L_Josuneds7_NEWAddProps_List = {} end
hook.Add("PlayerSpawnedVehicle","S_L_Josuneds7_NEWAddProps", function( ply, vehicle )
    if vehicle:GetModel( ) == "models/dannio/ds7.mdl" then
        // Create the actual prop
        local prop1 = ents.Create( "prop_dynamic_ornament" )
        if ( !IsValid( prop1 ) ) then return end
        prop1:SetModel( "models/dannio/ds7_part.mdl" )
	...
	--prop1:Fire("SetParentAttachment","vehicle_engine")
        ...
        table.insert(S_L_Josuneds7_NEWAddProps_List, {vehicle, prop1})
...
hook.Add("Think", "S_L_Josuneds7_NEWSyncChanges", function()
	if S_L_17_NEWAddProps_List and (!S_L_Josuneds7_NEWSyncChanges_Timer or CurTime() >= S_L_Josuneds7_NEWSyncChanges_Timer) then
		for k, v in pairs(S_L_Josuneds7_NEWAddProps_List) do
			if IsValid(v[2]) then
				...
				S_L_Josuneds7_NEWAddProps_List[k] = nil
			end
		end
		S_L_Josuneds7_NEWSyncChanges_Timer = CurTime()+0.5
	end
end)