if CLIENT then
	surface.CreateFont( "SGMSpeedometer_aventador_cop", {
	size = 48,
	weight = 1000,
	antialias = true,
	additive = false,
...
local SpeedometerData = {
	["models/sentry/aventador_cop_new.mdl"] = {
...
local function DrawSpeedometer()
	...
	for _, vehicle in pairs(NearbyVehicles) do
		...
		surface.SetFont("SGMSpeedometer_aventador_cop")
		...
		if data.mphpos then
			...
			cam.Start3D2D(mphpos, mphang, data.mphscale)
				draw.SimpleText( mph, "SGMSpeedometer_aventador_cop", 0, 0, data.mphcolor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
			cam.End3D2D()
		...
		if data.kmhpos then
			...
			cam.Start3D2D(kmhpos, kmhang, data.kmhscale)
				draw.SimpleText( kmh, "SGMSpeedometer_aventador_cop", 0, 0, data.kmhcolor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
			cam.End3D2D()
...
hook.Add("PostDrawTranslucentRenderables", "SGM_DrawSpeedometers_aventador_cop", DrawSpeedometer)

timer.Create("SGM_UpdateSpeedometers_aventador_cop", 1, 0, function()
if CLIENT then
